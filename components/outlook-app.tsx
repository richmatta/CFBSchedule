...
const formatSp = (value: number | null | undefined, signed = false) => value == null ? '—' : `${signed && value > 0 ? '+' : ''}${value.toFixed(1)}`;
    ...
    <main>
      {view==='setup'?<>
        ...
        {data&&!loading&&view==='schedule'&&<>
          ...
          <section className="sp-panel" aria-label="Team SP+ ratings"><div className="sp-heading"><h2>{team} SP+</h2><p>Latest available for {year} · Publication date not provided</p></div><dl className="sp-metrics"><div><dt>SP+</dt><dd>{formatSp(data.spRatings[team]?.overall,true)}</dd></div><div><dt>Offensive SP+</dt><dd>{formatSp(data.spRatings[team]?.offense)}</dd></div><div><dt>Defensive SP+</dt><dd>{formatSp(data.spRatings[team]?.defense)}</dd></div></dl><p className="sp-note">Higher is better for overall and offense; lower is better for defense. — means unavailable.</p></section>
